...
class Theme
{
    ...
    public static function init($theme)
    {
        ...
        // Do not merge common pages and menus for server side docs intances
        ...
        $actual_path = $_THEME_PATH;
        $general_config = include $actual_path . '/dist/config/general.php';
        $menu_config = include $actual_path . '/dist/config/menu.php';
        $pages_config = include $actual_path . '/dist/config/pages.php';

        $common_config['product'] = array_replace_recursive($common_config['product'], $general_config['product']);
        $common_config['meta'] = array_replace_recursive($common_config['meta'], $general_config['meta']);
        $theme_config['menu'] = array_replace_recursive($menu_config, $theme_config['menu']);
        $theme_config['pages'] = array_replace_recursive($pages_config, $theme_config['pages']);

        // Server side docs integration
        if (!empty(self::$product) && isset(self::$config['products']) && isset(self::$config['products'][self::$product])) {
            $theme_config['product'] = $theme_config['products'][self::$product];
            $theme_config['layout']['docs']['logo'] = $theme_config['layout']['docs']['logo'][self::$product];
            $theme_config['assets']['favicon'] = $theme_config['assets']['favicon'][self::$product];
        }
    ...
    public static function run()
    {
        ...
        if (isset($_REQUEST['layout-marker']) && $_REQUEST['layout-marker']) {
            self::setViewMode('export');
        }
    ...
    public static function isFrameworkDocsMode()
    {
        return !empty(self::$product);
    }

    public static function isFrameworkStarterkitAvailable()
    {
        $frameworks = Theme::getOption("general", "frameworks");

        foreach ($frameworks as $name => $framework) {
            if (isset($framework["themes"][self::getName()])) {
                return true;
            }
        }
    ...
    public static function getVendorFiles($type)
    {
        ...
        $globalVendors = Theme::getOption('assets', 'vendors');
    ...
    {
        return count(self::getVendorFiles($type)) > 0;
    }
    ...
    public static function checkPageByPath($path)
    {
        $pageConfig = self::getPageOptionsByPath($path);
    ...
    public static function getView($path, $params = array(), $once = false, $createPartialFileInExport = true)
    {
        ...
        // Include view
        self::includeView($actual_path, $params, $once, $createPartialFileInExport);
    }

    public static function getRootView($path, $params = array(), $once = false)
    {
        self::getView($path, $params, $once, false);
    }
    ...
    public static function includeView($path, $params = array(), $once = false, $createPartialFileInExport = true)
    {
        if (!file_exists($path)) {
        ...
        if (isset($_REQUEST['layout-marker']) && $createPartialFileInExport) {
            preg_match('/dist\/view\/(.*?)\.php$/', $path, $matches);
            if (!empty($matches)) {
                echo "\r\n" . '<!--layout-partial-start:' . htmlentities($matches[1]) . '-->' . "\r\n";
                if ($once === true) {
                    include_once($path);
                } else {
                    include($path);
                }
                echo "\r\n" . '<!--layout-partial-end:' . htmlentities($matches[1]) . '-->' . "\r\n";
            }
        } else {
            if ($once === true) {
                include_once($path);
    ...
    {
        $file = ltrim($file, '/');
        return self::getAssetsUrlPath() . 'media/' . $file;
    }
    ...
    {
        if ((isset($_REQUEST['rtl']) && $_REQUEST['rtl'] == 1) || self::getOption('layout', 'app/general/rtl') === true) {
            return true;
        } else {
            return false;
        }
    ...
    {
        if (self::hasOption('layout', 'app/general/dark-mode-enabled')) {
            return (bool) self::getOption('layout', 'app/general/dark-mode-enabled');
        }
    ...
    public static function getCurrentMode()
    {
        if (self::isDarkModeEnabled() === true) {
            ...
            if (self::hasOption('layout', 'app/general/mode')) {
                return self::getOption('layout', 'app/general/mode');
            }

            if (self::getOption('layout', 'main/dark-mode-default') === true) {
                return "dark";
            }
        }
    }
    ...
    {
        if (self::hasOption('layout', 'app/general/page-bg-white')) {
            return (bool) self::getOption('layout', 'app/general/page-bg-white');
        }

        return (bool) self::getOption('layout', 'main/page-bg-white');
    }

    public static function getPageUrl($path, $mode = null)
    {
    ...
    public static function getPackageReferences()
    {
        ...
        $references = self::getOption('general', 'references');
    ...
    public static function getChangelogInfo()
    {
        ...
        uasort($changelog, function ($a, $b) {
            $a = date_create_from_format('d F, Y', $a['date']);
            $b = date_create_from_format('d F, Y', $b['date']);
    ...
    public static function getImageDarkUrl($path)
    {
        $path = str_replace(".svg", "-dark.svg", $path);
        $path = str_replace(".png", "-dark.png", $path);
        $path = str_replace(".jpg", "-dark.jpg", $path);

        $path_dark_path = Theme::getMediaPath() . $path;
        $path_dark_url_path = Theme::getMediaUrlPath() .  $path;
    ...
    public static function hasImageDarkUrl($path)
    {
        return self::getImageDarkUrl($path) ? true : false;
    }
    ...
    public static function getImageUrl($path, $file = "")
    {
        ...
        return Theme::getMediaUrlPath() . $path;
    }

    public static function hasImageUrl($path)
    {
        return self::getImageUrl($path) ? true : false;
    }
    ...
    public static function getIllustrationDarkUrl($file)
    {
        $folder = 'illustrations/' . Theme::getOption('layout', 'illustrations/set');
        $folder = ltrim($folder, '/');
        $folder = rtrim($folder, '/');
    ...
    public static function hasIllustrationUrl($path)
    {
        return self::getIllustrationUrl($path) ? true : false;
    }

    public static function pagePathStartsWith($value)
    {
        return substr(Theme::getPagePath(), 0, strlen($value)) === $value;
    }

    public static function getPresetCssFileName($path)
    {
        $new_path = "";

        if (self::getOption("layout", "app/general/theme-preset") !== '' && self::getOption("layout", "app/general/theme-preset") !== "default") {
            $preset = self::getOption("layout", "app/general/theme-preset");

            if (strpos($path, "plugins.bundle.css") !== false) {
                $new_path = str_replace("plugins.bundle.css", "plugins." . $preset . ".bundle.css", $path);
            } else if (strpos($path, "style.bundle.css") !== false) {
                $new_path = str_replace("style.bundle.css", "style." . $preset . ".bundle.css", $path);
            }
        }

        if (!empty($new_path) && file_exists(Theme::getAssetsPath() . $new_path) === true) {
            return $new_path;
        } else {
            return $path;
        }
    ...
    {
        // for generating framrwork specific export
        $framework = $_REQUEST['framework'] ?? 'html';
        ...
        if (!empty($framework) && $framework !== "html") {
            return self::getFrameworkSvgIconIncludeCode($framework, $path, implode(' ', $cls));
        }

        $svg_content = file_get_contents($full_path);

        if (!empty($svgClass)) {
            $svg_content = str_replace("<svg ", "<svg class=\" . $svgClass .  \"", $svg_content);
        }
    ...
    public static function getFrameworkSvgIconIncludeCode($framework, $path, $class = '')
    {
        $content = '';

        if ($framework === 'django') {
            $content = "{% getSvgIcon 'assets/media/$path' '$class' %}";
        }
        ...
        if ($framework === 'asp.net-core') {
            $content = "@Html.Raw(KTTheme.getSvgIcon(\"/assets/media/$path\", \"$class\"))";
        }

        if ($framework === 'blazor') {
            $content = "@((MarkupString)KTTheme.getSvgIcon(\"$path\", \"$class\"))";
        }
