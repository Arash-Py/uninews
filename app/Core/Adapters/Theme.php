...
class Theme extends \App\Core\Theme
{
    ...
    public static function getView($path, $params = array(), $once = false, $createPartialFileInExport = true)
    {
        ...
        if (Str::startsWith($path, 'layout')) {
            $path = str_replace('layout/', 'layout/', $path);
        }
    ...
    public static function getOption($scope, $path = false, $default = null)
    {
        ...
        // Map the config path
        if (array_key_exists($scope, config('general', []))) {
            $scope = 'general.' . $scope;
        }
        ...
        $themeConfig = config($scope . $deepPath, $default);

        // check if it is a callback
        if (is_callable($themeConfig) && !is_string($themeConfig)) {
    ...
    public function initConfig()
    {
        ...
        $configPath = 'pages.' . str_replace('/', '.', Theme::getPagePath());
        $pageConfig = collect(config($configPath));

        // Merge group config with child config
        $pageGroupOptions = Theme::getPageGroupOptions(config('pages'), Theme::getPagePath());
        if ($pageGroupOptions) {
            $overridenConfig = $pageConfig->replaceRecursive($pageGroupOptions);
            config([$configPath => $overridenConfig->all()]);
        }

        $generalConfig = collect(config('general'));
        // Merge general config with page level config
        config(['general' => $generalConfig->replaceRecursive(config($configPath))->all()]);
    }
